...
export class ContextInjector extends EventEmitter {
    ...
    verifySymbol(symbolName: string, expectedType?: string): VerificationResult {
        if (!this.assimilator) {
            return { valid: true, exists: false, suggestions: ['Assimilator not initialized'], confidence: 0, issues: [] };
        }
     ...
     */
    async getVerifiedContext(query: string, maxTokens: number = 10000): Promise<string> {
        if (!this.assimilator) return '';
        const context = await this.assimilator.getRelevantContext(query, maxTokens);
        return context.map(c => JSON.stringify(c)).join('\n\n');
    }
    ...
    async buildContext(
        ...
        const context: ContextPayload = { task };
        ...
        if (this.assimilator) {
            const verifiedContext = await this.getVerifiedContext(prompt, 5000);
            if (verifiedContext) {
    ...
    private addErrorSolution(error: ErrorContext): void {
        ...
        const existing = this.backpack.errorSolutions.find(
            s => s.errorPattern === error.message
        );
...
export const createContextInjector = getContextInjector;
