...
export type DistilledKnowledgeSummary = DistilledKnowledge;
        ...
        const dfs = (node: string): void => {
            ...
            for (const edge of edges) {
                if (!visited.has(edge.to)) {
                    ...
                    // Намерен е цикъл
                    const cycleStart = traversalPath.indexOf(edge.to);
                    if (cycleStart !== -1) {
                        const cycle = traversalPath.slice(cycleStart).map(p =>
