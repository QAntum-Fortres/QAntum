...
export class OrderBookDepthEngine {
    ...
    public getHeatmapData(symbol: string, exchange: string): DepthHeatmapData | null {
        ...
        return {
            symbol, exchange, bidWall, askWall,
    ...
    public connectAll(): void {
        for (const symbol of this.config.symbols) this.connectBinanceWS(symbol);
    }
    ...
    private createFallbackSimulation(side: 'buy' | 'sell', amountUSD: number): ExecutionSimulation {
        return {
            side, requestedAmountUSD: amountUSD,
            avgFillPrice: 0, bestPrice: 0, worstPrice: 0,
            effectiveSlippage: 0.5,
            filledQuantity: 0, totalCostUSD: 0,
            levelsConsumed: 0, depthAvailableUSD: 0,
