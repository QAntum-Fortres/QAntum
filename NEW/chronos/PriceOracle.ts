...
export interface ChronosConfig {
  predictionHorizonSeconds: number;   // How far ahead to predict (default 30)
  confidenceThreshold:      number;   // Minimum confidence to emit signal (0–1)
  maxRiskPercent:           number;   // Signals above this VaR are 'extreme'
  updateIntervalMs:         number;   // How often predictions refresh
  simulationRuns:           number;   // Monte Carlo paths
}
...
export interface PricePrediction {
  ...
  priceChangePct:  number;   // Predicted % change over horizon
  ...
  // Interval & statistics
  upperBound:      number;   // 95th percentile price
  lowerBound:      number;   // 5th percentile price
  volatility:      number;   // σ (annualised)
  varPercent:      number;   // Value-at-Risk %
...
const DEFAULT_CHRONOS_CONFIG: ChronosConfig = {
...
export class PriceOracle extends EventEmitter {
  ...
  private cfg:        ChronosConfig;
  private cache:      Map<string, PricePrediction> = new Map();
   ...
   */
  public getPrediction(symbol: string): PricePrediction | null {
    const pred = this.cache.get(symbol);
    if (!pred) return null;
    if (pred.confidence < this.cfg.confidenceThreshold) return null;
    return pred;
  }
  ...
  ): { shouldProceed: boolean; riskAssessment: string; riskScore: number } {
  ...
  public feedPrice(symbol: string, price: number): void {
   ...
   */
  private buildPrediction(symbol: string, currentPrice: number): PricePrediction {
    ...
    const trend: PricePrediction['trend'] =
      priceChangePct > 0.05 ? 'bullish' :
      priceChangePct < -0.05 ? 'bearish' : 'neutral';

    const riskLevel: PricePrediction['riskLevel'] =
      varPercent > this.cfg.maxRiskPercent   ? 'extreme' :
