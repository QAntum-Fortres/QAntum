...
const CONFIG = {
    ...
    ignoreDirs: ['node_modules', 'dist', '.git', 'coverage', 'logs', '.purge-backup', '.refactor-backup', '.ascension-backup', 'NEW'],
    backupDir: '.ascension-backup',

    // Rename mappings
    renameMappings: {
        classes: [
            { from: 'QAntum', to: 'Aeterna' },
            { from: 'Qantum', to: 'Aeterna' },
            { from: 'MM', to: 'AE' }  // Alpha/Aeterna Engine
        ],
        interfaces: [
            { from: 'IQAntumConfig', to: 'IAeternaConfig' },
            { from: 'IQAntumOptions', to: 'IAeternaOptions' },
            { from: 'IQAntum', to: 'IAeterna' }
        ],
        functions: [
            { from: 'createQAntum', to: 'createAeterna' },
            { from: 'initQAntum', to: 'initAeterna' }
        ],
        variables: [
            { from: 'qAntum', to: 'aeterna' },
            { from: 'qantum', to: 'aeterna' }
...
${c.bright}WHAT IT DOES:${c.reset}
  Phase 1: Semantic rename of all QAntum → AETERNA
  Phase 2: Version bump to v1.0.0-AETERNA
