...
export interface ArbitrageOpportunity {
    id: string;
    symbol: string; // e.g., 'BTC-ETH-USDT' (triangular) or 'BTC-USDT' (spatial)
...
export interface ArbitrageConfig {
    ...
    exchangeFeePercent: number; // Average taker fee (e.g., 0.1% = 0.001)
...
export class ArbitrageLogic {
    ...
    private readonly CERTAINTY_THRESHOLD = 99.9;
     ...
     */
    public analyzeOpportunities(rawSpreads: any[]): ArbitrageOpportunity[] {
     ...
     */
    public getViableOpportunities(opportunities: ArbitrageOpportunity[]): ArbitrageOpportunity[] {
    ...
    private calculateSpatialArbitrage(spread: any): ArbitrageOpportunity {
        ...
        if (netProfit > 0) {
            ...
            if (netProfitPercent > this.config.minProfitPercent * 2) confidence += 5;
            if (netProfitPercent > this.config.minProfitPercent * 4) confidence += 4;

            // Penalty: Tiny margins
            if (netProfitPercent < 0.2) confidence -= 20;
