...
export class HotSwapModuleLoader extends EventEmitter {
  ...
  registerModule(name: string, instance: object, description?: string): SwappableModule {
    ...
    const methodNames = Object.getOwnPropertyNames(Object.getPrototypeOf(instance)).filter(
      (name) =>
        name !== 'constructor' && typeof (instance as Record<string, unknown>)[name] === 'function'
    );
  ...
  async swap(methodId: string, alternativeId: string): Promise<boolean> {
    ...
    try {
      ...
      this.emit('swapFailed', { methodId, alternativeId, error: error.message });
    ...
    try {
      ...
      if (activeAlternative) {
        ...
        activeAlternative.successRate =
          (activeAlternative.callCount - activeAlternative.errorCount) /
          activeAlternative.callCount;
      }
  ...
  private updateAlternativeTime(alternative: MethodAlternative, executionTime: number): void {
    const alpha = 0.2;
    alternative.avgExecutionTime =
      alternative.avgExecutionTime * (1 - alpha) + executionTime * alpha;
  }
  ...
  getModuleMethods(moduleId: string): SwappableMethod[] {
    return Array.from(this.methods.values()).filter((m) => m.moduleId === moduleId);
  }
  ...
  getStats(): typeof this.stats & {
    ...
    const activeSwaps = Array.from(this.methods.values()).filter(
      (m) => m.activeAlternativeId !== undefined
    ).length;
