...
export class CriticAgent extends BaseAgent {
   ...
   */
  async reviewResult(result: TaskResult, task?: SwarmTask): Promise<CriticFeedback> {
    ...
    if (result.confidence !== undefined && result.confidence < this.qualityThreshold) {
      issues.push(
        `Low confidence: ${(result.confidence * 100).toFixed(1)}% (threshold: ${(this.qualityThreshold * 100).toFixed(1)}%)`
      );
    ...
    const correction = issues.length > 0 ? this.suggestCorrection(issues, task, result) : undefined;
   ...
   */
  private suggestCorrection(issues: string[], task?: SwarmTask, result?: TaskResult): string {
    ...
    return suggestions.length > 0 ? suggestions.join('. ') : 'Review task parameters and retry';
  }
