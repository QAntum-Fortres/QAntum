...
export class SandboxExecutor extends EventEmitter {
  ...
  async validateMutation(
    ...
    const isMalicious = sandboxResult.violations.some(
      (v) => v.type === 'process' || v.type === 'filesystem' || v.type === 'network'
    );

    // Determine if mutation is unstable
    const isUnstable =
      sandboxResult.violations.some((v) => v.type === 'timeout' || v.type === 'memory') ||
      !!sandboxResult.error;
    ...
    const sandbox: Record<string, unknown> = {
      ...
      setTimeout: (fn: Function, ms: number) => {
        if (ms > policy.maxExecutionTime) {
          const violation = self.createViolation(
            'timeout',
            'setTimeout exceeds limit',
            `Requested: ${ms}ms`
          );
      ...
      {},
      {
        get: (_target, prop) => {
          const violation = self.createViolation(
            ...
            `Attempted to access process.${String(prop)}`
          );
          violations.push(violation);
          self.handleViolation(violation);
          return undefined;
        ...
        {},
        {
          get: (_target, prop) => {
            const violation = self.createViolation(
              ...
              `Attempted to access fs.${String(prop)}`
            );
            violations.push(violation);
            self.handleViolation(violation);
            return undefined;
