...
export class BrowserPoolManager extends EventEmitter {
  ...
  constructor(config?: BrowserPoolConfig) {
    ...
    this.log(
      `Browser pool configured for ${this.config.maxBrowsers} browsers (${cpuCores} CPU cores detected)`
    );
  }
  ...
  private async cleanup(): Promise<void> {
    ...
    for (const context of this.contexts.values()) {
      ...
      const idleTime = context.lastUsed ? now - context.lastUsed.getTime() : age;
  ...
  getStats(): typeof this.stats & {
    ...
    const available = Array.from(this.contexts.values()).filter(
    ...
    return {
      ...
      utilization: this.stats.currentContexts > 0 ? busy / this.stats.currentContexts : 0,
    };
  }
