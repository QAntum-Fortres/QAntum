...
export async function processGenesisTestJob(
  ...
  // Check axiom count
  if (limits.maxAxiomsPerReality !== -1 && realitySpec.axioms.length > limits.maxAxiomsPerReality) {
    throw new Error(
  ...
  if (config.collapseAfterRun) {
    ...
    console.log(
      `[GenesisProcessor] Reality ${manifestation.id} left active for future observations`
    );
      ...
      setTimeout(
        async () => {
          try {
            await genesisRealityProvider.collapseReality(manifestation.id);
            console.log(
              `[GenesisProcessor] Reality ${manifestation.id} auto-collapsed after timeout`
            );
          } catch {}
        },
...
export function startGenesisWorker(): Worker<GenesisTestJobData> {
  const worker = new Worker<GenesisTestJobData>('genesis-tests', processGenesisTestJob, {
    connection: redis,
