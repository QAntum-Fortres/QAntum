...
const BASE_MUTATION_PROBABILITIES: Record<MutationType, number> = {
  ...
  CLASS_ADDED: 0.3, // New features, variants
  CLASS_REMOVED: 0.25, // Cleanup, refactoring
  ID_CHANGED: 0.15, // Framework migrations
  ID_REMOVED: 0.1, // Accessibility improvements
  ELEMENT_MOVED: 0.2, // Layout changes
  ...
  TEXT_CHANGED: 0.4, // Content updates, i18n
...
export class NStepLookAheadSimulator extends EventEmitter {
  ...
  constructor(
    ...
    this.mutationProbabilities = new Map(
      Object.entries(BASE_MUTATION_PROBABILITIES) as [MutationType, number][]
    );
  ...
  private getBaseSurvivalChance(selectorType: SelectorType, mutationType: MutationType): number {
    ...
    const survivalMatrix: Record<SelectorType, Record<string, number>> = {
      ...
      ID: {
        default: 0.7,
        ID_CHANGED: 0.2,
        FRAMEWORK_MIGRATION: 0.3,
      },
      CLASS: {
        default: 0.4,
        CLASS_RENAMED: 0.15,
        FRAMEWORK_MIGRATION: 0.1,
      },
      TEXT_CONTENT: {
        default: 0.5,
        TEXT_CHANGED: 0.1,
    ...
    const typeMatrix = survivalMatrix[selectorType] || { default: 0.5 };
  ...
  private checkHistoricalSurvival(
    ...
    const wasAffected = relevantMutations.some((m) =>
      m.affectedSelectors.some((affected) => this.selectorTypesMatch(selector.value, affected))
    );
  ...
  private determineWinner(
    ...
    for (let i = 0; i < selectors.length && i < this.survivalScores.length; i++) {
      ...
      const compositeScore =
        survivalRate * 0.5 + selector.stability * 0.3 + selector.survivalProbability * 0.2;
  ...
  quickSurvivalCheck(selector: SelectorDNA): number {
    ...
    for (let i = 0; i < this.scenarios.length; i++) {
      ...
      const survives = this.getSelectorVulnerabilities(selector.type).every(
        (vuln) => !scenario.mutations.some((m) => m.type === vuln)
      );
