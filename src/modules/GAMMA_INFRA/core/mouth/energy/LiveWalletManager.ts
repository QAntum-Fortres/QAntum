...
╚═══════════════════════════════════════════════════════════════════════════╝
  ...
  public initializeVault(masterPassword: string): boolean {
    ...
    if (!(hasUpper && hasLower && hasNumber && hasSpecial)) {
      console.error(
        '[Fortress] ❌ Password must contain uppercase, lowercase, number, and special character'
      );
      return false;
    }
  ...
  public unlock(masterPassword: string): boolean {
    ...
    if (!this.fortress.verifyPassword(masterPassword, config.masterPasswordHash)) {
      this.failedAttempts++;
      console.error(
        `[Fortress] ❌ Invalid password. Attempt ${this.failedAttempts}/${this.maxFailedAttempts}`
      );
