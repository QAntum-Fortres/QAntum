...
// Logger not available in this context, using console
...
`;
    ...
    /**
     * 🕐 Human-like delay (for trading)
     */
    async humanDelay(ms: number): Promise<void> {
        const jitter = ms * 0.2 * (Math.random() - 0.5);
        return new Promise(resolve => setTimeout(resolve, ms + jitter));
    }

    /**
     * 💰 Humanize order amount (avoid round numbers)
     */
    humanizeOrderAmount(amount: number): { modifiedAmount: number; jitter: number } {
        const jitter = amount * 0.001 * (Math.random() - 0.5);
        return { modifiedAmount: amount + jitter, jitter };
    }

    /**
     * 💵 Humanize order price
     */
    humanizeOrderPrice(price: number, isBuy: boolean): number {
        const jitter = price * 0.0001 * (Math.random() - 0.5);
        return price + jitter;
    }
...
export function createBiometricProfile(
    ...
    // Access config via public getter or type assertion if needed
    // For now assuming the preset application updated the internal config
    // We will reconstruct the profile object manually rather than accessing private prop

    const generatedProfile: BiometricProfile = {
        profileId: 'bio_' + hash.substring(0, 16),
        handedness: seedNum % 10 < 9 ? 'right' : 'left',
        ...
        config: BIOMETRIC_PRESETS[presetName] as MotionConfig // Use the preset directly
    };
