...
export async function checkEntitlements(
  ...
  const usageCount = await prisma.testRun.count({
    where: {
      project: { tenantId },
 ...
 */
export async function checkGhostMode(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<void> {
  ...
  if (!entitlements) return; // Already sent error response
 ...
 */
export async function checkSelfHealing(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<void> {
  await checkEntitlements(request, reply);
 ...
 */
export async function checkParallelism(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<void> {
  await checkEntitlements(request, reply);
 ...
 */
export async function checkTestExecution(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<void> {
  await checkEntitlements(request, reply);
 ...
 */
export function getEntitlementsForPlan(plan: string) {
  return PLAN_ENTITLEMENTS[plan as keyof typeof PLAN_ENTITLEMENTS] || PLAN_ENTITLEMENTS.STARTER;
}
 ...
 */
export async function syncTenantEntitlements(tenantId: string, plan: string): Promise<void> {
