...
class GooglePage {
  ...
  async getSearchBox() {
    const allSelectors =
      'input[name="q"], textarea[name="q"], textarea[title="Search"], input[title="Search"]';

    try {
      const element = await this.driver.wait(until.elementLocated(By.css(allSelectors)), 10000);
