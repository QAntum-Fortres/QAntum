...
export interface FingerprintConfig {
  /** Seed for deterministic noise (same seed = same fingerprint per session) */
  seed?: string;
  /** Enable canvas noise injection (default: true) */
  canvasNoise: boolean;
  /** Enable WebGL parameter spoofing (default: true) */
  webglNoise: boolean;
  /** Enable AudioContext fingerprint spoofing (default: true) */
  audioNoise: boolean;
  /** Enable ClientRects noise (default: true) */
  rectNoise: boolean;
  /** Enable screen resolution spoofing (default: true) */
  screenSpoof: boolean;
  /** Enable font enumeration masking (default: true) */
  fontMask: boolean;
  /** Noise intensity 0-1 (higher = more disguised but potentially detectable) */
  noiseIntensity: number;
}

export interface FingerprintIdentity {
  /** Unique per-session identity hash */
  identityHash: string;
  ...
  /** Audio noise offset */
  audioOffset: number;
  /** Rect noise magnitude */
  rectNoiseMag: number;
  /** Allowed fonts subset */
  fontSubset: string[];
}
...
export class FingerprintInjector extends EventEmitter {
  private config: FingerprintConfig;
  private identity: FingerprintIdentity;
   ...
   */
  private generateIdentity(seed?: string): FingerprintIdentity {
   ...
   */
  public rotateIdentity(newSeed?: string): void {
   ...
   */
  public async patchPage(page: any): Promise<void> {
    ...
    await page.addInitScript((params: any) => {
      ...
      if (config.canvasNoise) {
        ...
        HTMLCanvasElement.prototype.toBlob = function(cb: BlobCallback, type?: string, quality?: number): void {
      ...
      if (config.webglNoise) {
        const spoofWebGL = (proto: any) => {
          ...
          proto.getParameter = function(param: number) {
            ...
            if (param === 0x9245) return identity.webgl.vendor;
            // UNMASKED_RENDERER_WEBGL
            if (param === 0x9246) return identity.webgl.renderer;
            // VENDOR
            if (param === 0x1F00) return identity.webgl.vendor;
            // RENDERER
            if (param === 0x1F01) return identity.webgl.renderer;
          ...
          proto.getSupportedExtensions = function() {
            ...
            return exts.filter((e: string) =>
              !e.includes('WEBGL_debug') && !e.includes('debug_shaders')
            );
          };
        };
      ...
      if (config.audioNoise) {
        ...
        AudioBuffer.prototype.copyFromChannel = function(dest: Float32Array, channel: number, offset?: number) {
        ...
        AudioContext.prototype.createOscillator = function() {
          ...
          const origFreqValue = Object.getOwnPropertyDescriptor(
      ...
      if (config.rectNoise) {
        ...
        const addNoise = (rect: DOMRect): DOMRect => {
        ...
        Element.prototype.getClientRects = function() {
          ...
          obj.item = (idx: number) => result[idx] || null;
          return obj;
        };
      }
   ...
   */
  public async verifyPatches(page: any): Promise<Record<string, boolean>> {
    return await page.evaluate((expected: any) => {
  ...
  public getIdentity(): FingerprintIdentity {
...
let injectorInstance: FingerprintInjector | null = null;

export function getFingerprintInjector(config?: Partial<FingerprintConfig>): FingerprintInjector {
  if (!injectorInstance) {
