 ...
 * ║                                                                               ║
 * ║  Created: 2026-02-23 | Aeterna Prime v28.2.0 - Stealth Layer 2               ║
 * ╚═══════════════════════════════════════════════════════════════════════════════╝
 */
...
export interface StealthTLSConfig {
  /** Chrome version to emulate (default: 121) */
  chromeVersion: number;
  /** Rotate User-Agent randomly from pool (default: true) */
  rotateUA: boolean;
  /** Enable CycleTLS Golang bridge if available (default: true) */
  useCycleTLS: boolean;
  /** Browser locale for Accept-Language (default: 'en-US') */
  locale: string;
  /** Additional locales for Accept-Language header */
  extraLocales: string[];
  /** Platform to emulate */
  platform: 'Windows' | 'macOS' | 'Linux';
  /** Randomize cipher order within Chrome-valid set (default: true) */
  randomizeCiphers: boolean;
}

export interface StealthRequest {
  url: string;
...
export interface StealthResponse {
  ...
  headers: Record<string, string>;
  body: string;
  provider: 'cycletls' | 'native-stealth' | 'raw';
  ja3Hash?: string;
  timing: {
    dns: number;
    connect: number;
...
const CHROME_PROFILES: Record<number, BrowserProfile> = {
  121: {
...
export class StealthTLS extends EventEmitter {
  private config: StealthTLSConfig;
  private activeProfile: BrowserProfile;
  private cycleTLS: any = null;
  private requestCount: number = 0;
  private lastRotation: number = Date.now();

  constructor(config?: Partial<StealthTLSConfig>) {
    super();
    this.config = {
   ...
   */
  public async initCycleTLS(): Promise<boolean> {
    if (!this.config.useCycleTLS) return false;
   ...
   */
  public async request(req: StealthRequest): Promise<StealthResponse> {
    this.requestCount++;
   ...
   */
  public getHarmonizedHeaders(extra?: Record<string, string>): Record<string, string> {
   ...
   */
  public getPlaywrightArgs(): string[] {
    return [
   ...
   */
  private async cycleTLSRequest(req: StealthRequest, startTime: number): Promise<StealthResponse> {
   ...
   */
  private nativeStealthRequest(req: StealthRequest, startTime: number): Promise<StealthResponse> {
    const headers = this.getHarmonizedHeaders(req.headers);
    ...
    return new Promise((resolve, reject) => {
      ...
      (options as any).secureContext = secureContext;
   ...
   */
  private rawRequest(req: StealthRequest, startTime: number): Promise<StealthResponse> {
    const url = new URL(req.url);
  ...
  private shuffleCiphers(ciphers: string[]): string[] {
  ...
  private calculateTiming(startTime: number) {
...
let stealthInstance: StealthTLS | null = null;

export function getStealthTLS(config?: Partial<StealthTLSConfig>): StealthTLS {
  if (!stealthInstance) {
