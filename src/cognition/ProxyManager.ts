 ...
 * ║   • SOCKS5 support                                                           ║
 * ║                                                                               ║
 * ║  Created: 2026-02-23 | Aeterna Prime v28.3.0 - Phase 3: Autonomous Survival  ║
 * ╚═══════════════════════════════════════════════════════════════════════════════╝
 */
...
export interface ProxyConfig {
  /** List of proxy URLs (protocol://user:pass@host:port) */
  proxies: string[];
  /** Rotating proxy gateway URL (e.g., BrightData super proxy) */
  gatewayURL?: string;
  /** Gateway credentials */
  gatewayAuth?: { username: string; password: string; zone?: string };
  /** Health check interval ms (default: 300000 = 5 min) */
  healthCheckInterval: number;
  /** Max consecutive failures before removing proxy (default: 3) */
  maxFailures: number;
  /** Test URL for health checks (default: https://httpbin.org/ip) */
  testURL: string;
  /** Health check timeout ms (default: 10000) */
  testTimeout: number;
  /** Minimum time between rotations ms (default: 5000) */
  minRotationInterval: number;
  /** Enable geo-targeting (default: false) */
  geoTargeting: boolean;
  /** Preferred country codes */
  preferredCountries: string[];
  /** Max requests per proxy before forced rotation (default: 100) */
  maxRequestsPerProxy: number;
  /** Go direct (no proxy) when all proxies fail (default: true) */
  allowDirect: boolean;
}
...
export interface ProxyEntry {
  ...
  lastUsed: number;
  lastChecked: number;
  /** Weight for selection (higher = preferred) */
  weight: number;
}

export interface RotationResult {
  /** Previous proxy URL (or 'direct') */
  previousProxy: string;
  /** New proxy URL (or 'direct') */
  newProxy: string;
  /** Reason for rotation */
  reason: string;
  /** Alive proxy count */
...
export class ProxyManager extends EventEmitter {
  ...
  private pool: Map<string, ProxyEntry> = new Map();
  private activeProxy: ProxyEntry | null = null;
  private lastRotation: number = 0;
  private healthCheckTimer: ReturnType<typeof setInterval> | null = null;
  private using_gateway: boolean = false;
   ...
   */
  public addProxy(proxyUrl: string, meta?: { country?: string; city?: string }): void {
    if (this.pool.has(proxyUrl)) return;

    try {
      const parsed = new URL(proxyUrl);
      const entry: ProxyEntry = {
        url: proxyUrl,
        protocol: (parsed.protocol.replace(':', '') as ProxyEntry['protocol']) || 'http',
   ...
   */
  public async rotateIP(reason: string = 'manual'): Promise<RotationResult> {
    const now = Date.now();
    ...
    if (this.activeProxy && (reason === 'blocked' || reason === 'captcha')) {
      ...
      if (reason === 'blocked') this.stats.blockedIPs++;
      if (reason === 'captcha') this.stats.captchaTriggered++;
   ...
   */
  private selectBestProxy(): ProxyEntry | null {
    const alive = Array.from(this.pool.values()).filter(p =>
      p.alive && p.url !== this.activeProxy?.url
    );
    ...
    if (this.config.geoTargeting && this.config.preferredCountries.length > 0) {
      const geoCandidates = alive.filter(p =>
        p.country && this.config.preferredCountries.includes(p.country)
      );
      if (geoCandidates.length > 0) candidates = geoCandidates;
    }
    ...
    const scored = candidates.map(proxy => {
      ...
      if (idleTime > 60_000) score *= 1.2; // 1min idle bonus
    ...
    if (totalScore <= 0) return scored[0]?.proxy || null;
   ...
   */
  public getPlaywrightProxy(): { server: string; username?: string; password?: string } | undefined {
    if (!this.activeProxy) return undefined;
   ...
   */
  public getActiveProxyURL(): string | null {
   ...
   */
  public async applyToContext(browser: any): Promise<any> {
   ...
   */
  public recordRequest(success: boolean, latencyMs?: number): void {
   ...
   */
  private async checkProxy(proxy: ProxyEntry): Promise<void> {
   ...
   */
  private testRequest(proxy: ProxyEntry): Promise<{ ok: boolean; data?: string }> {
   ...
   */
  private parseGatewayAuth(): ProxyConfig['gatewayAuth'] {
    ...
    if (user && pass) return { username: user, password: pass, zone };
    return undefined;
  }
...
let proxyInstance: ProxyManager | null = null;

export function getProxyManager(config?: Partial<ProxyConfig>): ProxyManager {
  if (!proxyInstance) {
