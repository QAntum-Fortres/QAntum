...
export class GhostProtocol {
  ...
  endSession(sessionId: string): void {
    ...
    if (session) {
      ...
      console.log(
        `[GhostProtocol] Session ended: ${sessionId}, Duration: ${duration}ms, Requests: ${session.requestCount}`
      );
  ...
  async scrollHuman(
    ...
    while (scrolled < totalDistance) {
      const step = Math.min(this.randomBetween(50, 150), totalDistance - scrolled);
   ...
   */
  async clickHuman(sessionId: string, clickCallback: () => Promise<void>): Promise<void> {
    const session = this.sessions.get(sessionId);
    const profile = TIMING_PROFILES[session?.level || 'medium'];
