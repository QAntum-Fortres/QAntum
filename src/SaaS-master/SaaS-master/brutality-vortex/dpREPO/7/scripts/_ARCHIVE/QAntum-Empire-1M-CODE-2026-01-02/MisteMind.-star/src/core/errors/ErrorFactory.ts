 ...
 * @description Centralized Error Factory - Единна система за грешки в Aeterna Prime
 ...
 * @example
 * ```typescript
 * import { AeternaError, ErrorCodes } from '@/core/errors/ErrorFactory';
 * 
 * throw new AeternaError(ErrorCodes.NETWORK_TIMEOUT, {
...
/**
 * @class AeternaError
 * @description Централизирана грешка за цялата Aeterna система
 * 
 * @example
 * ```typescript
 * throw new AeternaError(ErrorCodes.NETWORK_TIMEOUT, {
 ...
 * ```
 */
export class AeternaError extends Error {
  ...
  constructor(
    ...
    if (cause) {
      ...
      if (cause instanceof AeternaError) {
   ...
   */
  withContext(additionalContext: Record<string, unknown>): AeternaError {
    return new AeternaError(
      this.code,
    ...
    try {
      ...
      if (result instanceof Promise) {
        return result.catch(error => {
          ...
          throw new AeternaError(defaultErrorCode, { args }, error as Error);
        });
      }
...
export class ErrorAggregator {
  private errors: AeternaError[] = [];

  add(error: AeternaError): void {
  ...
  getErrors(): AeternaError[] {
    return [...this.errors];
  }

  getByCategory(category: ErrorCategory): AeternaError[] {
    return this.errors.filter(e => e.category === category);
  }

  getBySeverity(minSeverity: ErrorSeverity): AeternaError[] {
    return this.errors.filter(e => e.severity >= minSeverity);
  }

  getMostSevere(): AeternaError | undefined {
    return this.errors.reduce((max, current) =>
      (current.severity > (max?.severity ?? -1)) ? current : max,
      undefined as AeternaError | undefined
    );
  }
