...
class AutoDocEngine {
  ...
  async run(commitMessage = null) {
    console.log(`
╔══════════════════════════════════════════════════════════════════════════════╗
║  ⚛️  AETERNA AUTO-DOC ENGINE v1.0.0                                           ║
  ...
  async updateChangelog(commitMessage) {
    ...
    if (headerEnd > 0) {
      ...
      if (!changelog.includes('# Changelog')) {
        changelog = `# Changelog\n\nAll notable changes to Aeterna are documented automatically.\n${newEntry}`;
      } else {
        changelog += newEntry;
      }
    }
...
<head>
  ...
  <meta name="version" content="${this.version}">
  <title>Changelog - Aeterna v${this.version}</title>
...
<body>
  ...
  <footer>
    <p>Aeterna v${this.version} | Last updated: ${this.timestamp}</p>
  </footer>
</body>
</html>`;
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="version" content="${this.version}">
  <title>Documentation - Aeterna v${this.version}</title>
  <link rel="stylesheet" href="styles.css">
</head>
<body>
  <header>
