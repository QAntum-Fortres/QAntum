...
╚═══════════════════════════════════════════════════════════════╝
`);
        console.log(`🔧 [HEALER] Anchor: ${anchor.id}`);
        console.log(`🔧 [HEALER] Change: ${change.changeType}`);
        console.log(`🔧 [HEALER] Previous: ${change.previousSelector}`);
        console.log('');
        ...
        for (const strategy of this.config.healingStrategies) {
            ...
            console.log(`🔧 [HEALER] Trying strategy: ${strategy}`);
            
            try {
                result = await this.tryStrategy(page, anchor, change, strategy);
                
                if (result.success) {
                    console.log(`✅ [HEALER] Success with ${strategy}!`);
                    console.log(`   New selector: ${result.newSelector}`);
                    console.log(`   Confidence: ${(result.confidence * 100).toFixed(1)}%`);
                }
            } catch (error: any) {
                console.log(`❌ [HEALER] ${strategy} failed: ${error.message}`);
            }
        }
    ...
    private async tryMLPrediction(
        ...
        console.log(`🔧 [ML] Using learned pattern: ${bestPattern.pattern}`);
        console.log(`🔧 [ML] Recommended strategy: ${bestPattern.successfulStrategy}`);
    ...
    private async refactorTests(
        ...
        console.log('🔧 [HEALER] Refactoring test files...');
        ...
        console.log(`🔧 [HEALER] Refactored ${refactoredFiles.length} files`);
    ...
    private async refactorFile(
        ...
        try {
            ...
            console.error(`   ❌ Failed to refactor: ${filePath}`);
            return false;
        }
    }
    ...
    private learnFromHealing(change: AnchorChange, result: HealingResult): void {
        ...
        console.log(`🧠 [LEARN] Pattern learned: ${pattern} → ${result.strategy}`);
        this.saveHealingData();
    }
    ...
    async startMonitoring(page: any, anchors: CognitiveAnchor[]): Promise<void> {
        console.log(`🔧 [MONITOR] Starting real-time monitoring for ${anchors.length} anchors`);
    ...
    private async checkForMutations(page: any, anchors: CognitiveAnchor[]): Promise<void> {
        try {
            ...
            if (mutations.length > 0) {
                console.log(`🔧 [MONITOR] Detected ${mutations.length} mutations`);
                ...
                for (const anchor of anchors) {
                    ...
                    if (!element?.found) {
                        console.log(`🔧 [MONITOR] Anchor broken: ${anchor.id}`);
