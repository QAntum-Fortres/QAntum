...
╚═══════════════════════════════════════════════════════════════╝
`);
        console.log(`🤖 [EXPLORER] Target: ${baseUrl}`);
        console.log(`🤖 [EXPLORER] Max Pages: ${this.config.maxPages}`);
        console.log(`🤖 [EXPLORER] Workers: ${this.config.parallelWorkers}`);
        console.log('');
        ...
        console.log(`   Pages discovered: ${this.siteMap.totalPages}`);
        console.log(`   Forms found: ${this.siteMap.totalForms}`);
        console.log(`   API endpoints: ${this.siteMap.totalApiEndpoints}`);
        console.log(`   Transaction flows: ${this.siteMap.transactionFlows.length}`);
    ...
    private async crawlerWorker(workerId: number): Promise<void> {
        ...
        const context = await this.browser.newContext({
            ...
            userAgent: 'Aeterna-Explorer/26.0'
        });
        ...
        while (this.urlQueue.length > 0 || this.activeWorkers > 0) {
            ...
            try {
                ...
                console.error(`🤖 [WORKER-${workerId}] Error crawling ${item.url}:`, error.message);
            }
    ...
    private async crawlPage(
        ...
        console.log(`🤖 [EXPLORER] Crawling: ${url} (depth: ${depth})`);
    ...
    private analyzeApiPatterns(): void {
        ...
        for (const [key, endpoint] of this.siteMap.apiEndpoints) {
            ...
            if (/^\d+$/.test(lastPart) || /^[a-f0-9-]{36}$/.test(lastPart)) {
                ...
                console.log(`🤖 [EXPLORER] Detected resource: ${resourceUrl}`);
            }
        }
    }
    ...
    private async saveSiteMap(): Promise<void> {
        ...
        console.log(`🤖 [EXPLORER] Sitemap saved: ${filepath}`);
    }
