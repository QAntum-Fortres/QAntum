...
`);
        ...
        logger.debug(`   ✅ Singularity Core v${this.singularityCore?.version}`);
        logger.debug(`   📁 Output directory: ${this.config.outputDir}`);
        logger.debug(`   ☁️ Providers: ${this.config.providers.join(', ')}`);
    }
    ...
    private async generateAndSaveTemplate(
        ...
        logger.debug(`   ✅ ${provider}/${templateType}: updated`);
    ...
    private static generateDockerTemplate(core: SingularityCore, vars: Record<string, any>): string {
        return `# AETERNA QA Framework v${core.version}
...
# Metadata
LABEL maintainer="AETERNA AI" \\
      version="${core.version}" \\
      description="AETERNA QA Framework - Singularity Core"
...
ENV NODE_ENV=production \\
    AETERNA_MIND_VERSION=${core.version} \\
...
services:
  AETERNA-mind:
    build:
      context: .
      dockerfile: Dockerfile
    container_name: AETERNA-mind-${vars.environment || 'prod'}
    image: AETERNA-mind:${core.version}
    ports:
  ...
  redis:
    ...
    container_name: AETERNA-mind-redis
    ports:
      - "6379:6379"
    volumes:
      - redis-data:/data
  ...
  prometheus:
    ...
    container_name: AETERNA-mind-prometheus
    ports:
      - "9090:9090"
    volumes:
  ...
  grafana:
    ...
    container_name: AETERNA-mind-grafana
    ports:
      - "3001:3000"
    environment:
...
Description: AETERNA QA Framework v${core.version} - Singularity Core
...
Resources:
  ...
  AeternaCluster:
    ...
    Properties:
      ClusterName: AETERNA-mind-\${Environment}
      CapacityProviders:
        - FARGATE
        - FARGATE_SPOT
  ...
  AeternaTaskDefinition:
    ...
    Properties:
      ...
      ContainerDefinitions:
        - Name: AETERNA-mind
          Image: !Sub '\${AWS::AccountId}.dkr.ecr.\${AWS::Region}.amazonaws.com/aeterna:${core.version}'
          PortMappings:
          ...
          Environment:
            ...
            - Name: AETERNA_MIND_VERSION
              Value: ${core.version}
          LogConfiguration:
            LogDriver: awslogs
            Options:
              awslogs-group: /ecs/aeterna
              awslogs-region: !Ref AWS::Region
              awslogs-stream-prefix: AETERNA-mind
          HealthCheck:
            Command:
              - CMD-SHELL
  ...
  AeternaService:
    ...
    Properties:
      ServiceName: AETERNA-mind-service
      Cluster: !Ref AeternaCluster
      TaskDefinition: !Ref AeternaTaskDefinition
  ...
  AutoScalingTarget:
    ...
    Properties:
      ...
      ResourceId: !Sub service/\${AeternaCluster}/\${AeternaService.Name}
  ...
  AutoScalingPolicy:
    ...
    Properties:
      PolicyName: AETERNA-mind-cpu-scaling
...
Outputs:
  ClusterArn:
    Value: !GetAtt AeternaCluster.Arn
  ServiceUrl:
...
Description: AETERNA Lambda Workers v${core.version}
...
Resources:
  AeternaWorker:
    ...
    Properties:
      FunctionName: AETERNA-mind-worker
  ...
  TestQueue:
    Type: AWS::SQS::Queue
    Properties:
      QueueName: AETERNA-mind-test-queue
      VisibilityTimeout: 900

  ResultsBucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub AETERNA-mind-results-\${AWS::AccountId}
`;
    }
...
spec:
  template:
    ...
    spec:
      ...
      timeoutSeconds: 900
      containers:
        - image: gcr.io/PROJECT_ID/aeterna:${core.version}
          ports:
            - containerPort: ${vars.port || 3000}
          env:
            - name: NODE_ENV
