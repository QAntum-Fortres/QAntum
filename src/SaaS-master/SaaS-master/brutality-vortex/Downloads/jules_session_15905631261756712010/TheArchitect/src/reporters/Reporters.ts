...
export class ConsoleReporter extends BaseReporter {
  ...
  async generate(): Promise<string> {
    ...
    logger.debug('\n');
    logger.debug(`${c.bright}${c.cyan}═══════════════════════════════════════════════════════════════${c.reset}`);
    logger.debug(`${c.bright}${c.cyan}   🧠 MIND ENGINE TEST REPORT${c.reset}`);
    logger.debug(`${c.bright}${c.cyan}═══════════════════════════════════════════════════════════════${c.reset}`);
    logger.debug('');

    // Summary
    logger.debug(`${c.bright}📊 SUMMARY${c.reset}`);
    logger.debug(`   Total:   ${c.bright}${summary.total}${c.reset}`);
    logger.debug(`   ${c.green}✓ Passed: ${summary.passed}${c.reset}`);
    logger.debug(`   ${c.red}✗ Failed: ${summary.failed}${c.reset}`);
    logger.debug(`   ${c.yellow}○ Skipped: ${summary.skipped}${c.reset}`);
    logger.debug(`   Duration: ${this.formatDuration(summary.duration)}`);
    logger.debug(`   Pass Rate: ${c.bright}${summary.passRate.toFixed(1)}%${c.reset}`);
    logger.debug('');
    ...
    if (failed.length > 0) {
      logger.debug(`${c.bright}${c.red}❌ FAILED TESTS${c.reset}`);
      logger.debug('');
      
      for (const test of failed) {
        logger.debug(`   ${c.red}✗${c.reset} ${test.name}`);
        if (test.error) {
          logger.debug(`     ${c.dim}${test.error.message}${c.reset}`);
        }
        logger.debug('');
      }
    }
    ...
    // Test list
    logger.debug(`${c.bright}📋 TEST RESULTS${c.reset}`);
    logger.debug('');
    ...
    for (const test of this.results) {
      ...
      logger.debug(`   ${icon} ${test.name} ${duration}`);
    }
