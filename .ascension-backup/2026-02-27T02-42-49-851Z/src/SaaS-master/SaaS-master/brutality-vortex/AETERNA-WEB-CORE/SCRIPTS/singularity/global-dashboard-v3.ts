...
`);
        ...
        logger.debug(`🌍 [DASHBOARD] HTTP Server: http://localhost:${this.config.port}`);
        logger.debug(`🌍 [DASHBOARD] WebSocket: ws://localhost:${this.config.wsPort}`);
        logger.debug(`🌍 [DASHBOARD] Theme: ${this.config.theme.toUpperCase()}`);
        logger.debug('');
    }
    ...
    private async startWebSocketServer(): Promise<void> {
        ...
        this.wsServer.on('connection', (ws) => {
            this.clients.add(ws);
            logger.debug(`🌍 [DASHBOARD] Client connected (${this.clients.size} total)`);
            ...
            ws.on('close', () => {
                ...
                logger.debug(`🌍 [DASHBOARD] Client disconnected (${this.clients.size} total)`);
            });
        });
    }
...
<head>
    ...
    <title>🌍 AETERNA - Global Dashboard V3</title>
    <style>
...
<body>
    <div class="header">
        <div class="logo">🌍 AETERNA - GLOBAL SWARM</div>
