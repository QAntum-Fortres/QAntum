...
const _child = _require ? _require('child_process') : { execSync: () => ({ success: false }), spawn: () => ({ on: () => { } }) };
const _crypto = _require ? _require('crypto') : { createHash: () => ({ update: () => ({ digest: () => ({ slice: () => '' }) }) }) };

const fs = _fs;
const path = _path;
const { execSync, spawn } = _child;
const _crypto_internal = _crypto;
const _process = (globalThis as any).process || { cwd: () => '/', env: {}, version: 'v0.0.0', argv: [] };

// ═══════════════════════════════════════════════════════════════════════════════
// CONFIGURATION - DYNAMIC RESOLUTION
// ═══════════════════════════════════════════════════════════════════════════════

// Detect actual root instead of hardcoded paths
const PROJECT_ROOT = _process.cwd();

const CONFIG = {
  rootPath: PROJECT_ROOT,
  srcPath: path.join(PROJECT_ROOT, 'src'),
  docsPath: path.join(PROJECT_ROOT, 'docs'),
  AeternaPath: 'C:\\AeternaQATool',
...
export const MODULE_REGISTRY = {
  ...
  'saas-platform': {
    ...
    exports: ['AeternaSaaSPlatform', 'LicenseManager', 'UsageMeter', 'PaymentEngine', 'SUBSCRIPTION_TIERS'],
    dependencies: ['stripe'],
...
export class ScriptRunner {
  ...
  hash(content: string): string {
    return _crypto_internal.createHash('sha256').update(content).digest('hex').slice(0, 16);
  }
...
export class DocumentationScript extends ScriptRunner {
  ...
  private generateMainReadme(): void {
    ...
    const content = `# 🧠 Aeterna Mind Engine Core
...
\`\`\`typescript
import { ${module.exports.join(', ')} } from '@aeterna/${id}';
\`\`\`
...
\`\`\`typescript
import { ${module.exports[0]} } from '@aeterna/${id}';
...
Aeterna uses a 5-layer architecture:
...
> Get up and running with Aeterna in 5 minutes.
...
git clone https://github.com/papica777-eng/AeternaQATool.git
...
\`\`\`typescript
import { GhostProtocolV2 } from '@aeterna/ghost-protocol-v2';
...
\`\`\`bash
# Get your license key from https://aeterna.dev
export AETERNA_LICENSE=QNTM-PRO-XXXX-XXXX-XXXX-SIG

# Or in code
const platform = new AeternaSaaSPlatform();
platform.validateAccess(process.env.AETERNA_LICENSE);
\`\`\`
...
All notable changes to Aeterna Mind Engine Core.
 ...
 * Aeterna Mind Engine Core - Barrel Export
...
export class DeployScript extends ScriptRunner {
  ...
  private deployNpm(): void {
    Object.keys(MODULE_REGISTRY).forEach(id => {
      this.log(`Publishing @aeterna/${id}...`, 'info');
...
export class AutomationEngine {
  ...
  async runPipeline(): Promise<void> {
    console.log('╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║     🔧 AETERNA AUTOMATION ENGINE                                              ║');
