...
const _events = _require ? _require('events') : { EventEmitter: class { emit() { }; on() { }; once() { }; removeListener() { }; } };
const _fs = _require ? _require('fs') : { promises: { mkdir: async () => { }, writeFile: async () => { }, writeFileSync: () => { } }, writeFileSync: () => { } };
const _path = _require ? _require('path') : { join: (...args: string[]) => args.join('/'), dirname: (p: string) => p };
