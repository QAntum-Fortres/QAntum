...
export class NeuralMapEngine {
    ...
    async createAnchor(
        ...
        console.log(`🧠 [NEURAL] Creating Cognitive Anchor: ${name}`);
        ...
        console.log(`🧠 [NEURAL] Anchor created with ${selectors.length} selector signals`);
        console.log(`   Top selector: ${selectors[0]?.selector} (${selectors[0]?.confidence}% confidence)`);
    ...
    async findElement(page: Page, anchorId: string): Promise<ElementHandle | null> {
        ...
        if (!anchor) {
            console.warn(`🧠 [NEURAL] Anchor not found: ${anchorId}`);
            return null;
        }
            ...
            try {
                ...
                if (element) {
                    ...
                    if (this.config.visualFingerprintEnabled) {
                        ...
                        if (!isMatch) {
                            console.log(`🧠 [NEURAL] Visual mismatch for ${signal.type}, trying next`);
                            continue;
                        }
                    }
        ...
        if (this.config.autoHealingEnabled) {
            console.log(`🧠 [NEURAL] All selectors failed, initiating self-healing...`);
    ...
    async buildDomAst(page: Page): Promise<DOMNode> {
        console.log(`🧠 [NEURAL] Building DOM AST...`);
        ...
        this.pageAst = ast;
        console.log(`🧠 [NEURAL] AST built with ${this.countNodes(ast)} nodes`);
        return ast;
    }
    ...
    private async selfHeal(page: Page, anchor: CognitiveAnchor): Promise<ElementHandle | null> {
        console.log(`🩹 [NEURAL] Self-healing anchor: ${anchor.name}`);
        ...
        console.error(`🩹 [NEURAL] Self-healing failed for: ${anchor.name}`);
    ...
    private async updateAnchorFromElement(
        ...
        console.log(`🩹 [NEURAL] Healed using: ${healingMethod}`);
    ...
    private loadAnchors(): void {
        ...
        try {
            if (fs.existsSync(filePath)) {
                ...
                console.log(`🧠 [NEURAL] Loaded ${this.anchors.size} anchors`);
            }
        } catch (error) {
            console.warn('🧠 [NEURAL] Could not load anchors');
        }
    }
    ...
    private saveAnchors(): void {
        ...
        try {
            ...
            console.error('🧠 [NEURAL] Could not save anchors');
        }
    }
}
