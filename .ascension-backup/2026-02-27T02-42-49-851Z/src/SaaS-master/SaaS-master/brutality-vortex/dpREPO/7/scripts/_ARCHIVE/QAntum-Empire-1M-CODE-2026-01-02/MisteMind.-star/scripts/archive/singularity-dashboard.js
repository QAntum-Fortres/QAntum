 ...
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║                                                                              ║
 * ║     🧠 AETERNA COMMAND CENTER v3.0 - THE SINGULARITY                          ║
...
const runCommand = (cmd, cwd = CONFIG.AeternaPath) => {
...
const apiHandlers = {
  ...
  'POST /api/ghost/browse': async (body) => {
    ...
    const scriptPath = path.join(CONFIG.AeternaPath, '_ghost_temp.js');
  ...
  'POST /api/browser/screenshot': async (body) => {
    ...
    const scriptPath = path.join(CONFIG.AeternaPath, '_ss_temp.js');
    fs.writeFileSync(scriptPath, script);
    const evidencePath = path.join(CONFIG.AeternaPath, 'evidence');
  ...
  'POST /api/enterprise/package': async () => {
    ...
    const result = await runCommand('npm pack', CONFIG.AeternaPath);
    addLog('BUILD', result.success ? 'Package created' : 'Package failed', result.success ? 'success' : 'error');
    return { success: result.success, output: result.output };
  },
  
  'GET /api/enterprise/status': async () => {
    const distPath = path.join(CONFIG.AeternaPath, 'dist');
    const protectedPath = path.join(CONFIG.AeternaPath, 'dist-protected');
  ...
  'POST /api/tools/surgeon': async (body) => {
    ...
    addLog('SURGEON', `Running AETERNA Surgeon: ${action}`, 'info');
    const result = await runCommand(`npx tsx scripts/aeterna-surgeon.ts --${action}`);
...
<head>
  ...
  <title>🧠 Aeterna Command Center v3.0</title>
  <style>
...
<body>
  <div class="app">
    <aside class="sidebar">
      <div class="logo"><h1>🧠 Aeterna</h1><div class="motto">"В Aeterna не лъжем."</div><div class="ver">Command Center v3.0</div></div>
      <nav class="nav">
      ...
      <div class="page" id="page-system">
        ...
        <div class="cards">
          ...
          <div class="card" style="grid-column:span 2"><div class="card-title"><span class="icon">💻</span> Command</div>
            <div class="input-group"><label>Execute (AeternaQATool)</label><input type="text" class="input" id="exec-cmd" placeholder="npm run ..." onkeypress="if(event.key==='Enter')execCmd()"></div>
...
server.listen(CONFIG.port, '0.0.0.0', () => {
  addLog('SYSTEM', 'Aeterna Command Center started', 'success');
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║     🧠 AETERNA COMMAND CENTER v3.0 - THE SINGULARITY                          ║');
  console.log('║     "В Aeterna не лъжем. Само истински стойности."                            ║');
  console.log('╠══════════════════════════════════════════════════════════════════════════════╣');
