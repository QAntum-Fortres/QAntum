/**
 * ⚛️📡 AETERNA SENTINEL LINK - CLOUD VERIFICATION HEARTBEAT
 * ═══════════════════════════════════════════════════════════════════════════════════════
 *
 *   ███████╗███████╗███╗   ██╗████████╗██╗███╗   ██╗███████╗██╗
 *   ██╔════╝██╔════╝████╗  ██║╚══██╔══╝██║████╗  ██║██╔════╝██║
 ...
 *
 *   Aeterna поддържа постоянна връзка със Sentinel сървър.
...
export class SentinelLink extends EventEmitter {
  ...
  private static readonly DEFAULT_SERVER = 'https://sentinel.aeterna.io';
  ...
  constructor(config: Partial<SentinelConfig>) {
    ...
    this.config = {
      ...
      encryptionKey: config.encryptionKey || 'Aeterna-Sentinel-Default-Key',
...
`);
  ...
  private async checkOfflineToken(): Promise<boolean> {
    try {
      const tokenPath = path.resolve('./.aeterna-offline-token');
  ...
  private disable(): void {
    console.log(`[SENTINEL] 🔒 Aeterna disabled`);
    this.isActive = false;
    this.isDormant = true;
  ...
  private encryptToken(token: OfflineToken): string {
    ...
    const key = crypto.scryptSync(this.config.encryptionKey, 'AeternaSalt', 32);
