 ...
 * Abstract Syntax Tree (AST) parsing. It enables AETERNA to "understand"
...
export class NeuralMapper {
    ...
    async takeContextPulse(): Promise<ContextPulse> {
        ...
        const elapsed = ((performance.now() - startTime) / 1000).toFixed(2);
        logger.debug(`✅ [NeuralMapper] Context Pulse complete in ${elapsed}s`);

        if (alerts.length > 0) {
            logger.debug('\n🚨 ALERTS:');
            alerts.forEach(a => logger.debug(`   ${a}`));
        }
    ...
    async exportBrainMap(outputPath: string): Promise<void> {
        ...
        logger.debug(`📁 Brain Map exported to: ${outputPath}`);
    }
