 ...
 * ║                                                                               ║
 * ║  Created: 2026-02-23 | Aeterna Prime v28.2.0 - Stealth Layer                  ║
 * ╚═══════════════════════════════════════════════════════════════════════════════╝
 */
...
export interface BezierConfig {
  /** Base movement speed in pixels/step (default: 2.5) */
  baseSpeed: number;
  /** Overshoot probability 0-1 (default: 0.75) */
  overshootProbability: number;
  /** Overshoot distance as fraction of total distance (default: 0.08-0.15) */
  overshootRange: [number, number];
  /** Micro-jitter amplitude in pixels (default: 0.3-1.5) */
  jitterRange: [number, number];
  /** Nervousness factor 0-1 — increases jitter & speed variance (default: 0.3) */
  nervousness: number;
  /** Number of steps for the movement curve (default: auto-calculated) */
  steps?: number;
  /** Hesitation pause before click in ms (default: 50-250) */
  hesitationRange: [number, number];
  /** Enable fatigue model — jitter increases over time (default: true) */
  enableFatigue: boolean;
  /** Fatigue buildup rate per minute (default: 0.01) */
  fatigueRate: number;
  /** Max fatigue multiplier (default: 2.5) */
  maxFatigue: number;
}
...
export interface MovementStep {
  ...
  phase: 'accelerate' | 'cruise' | 'decelerate' | 'overshoot' | 'correct' | 'hesitate';
}
...
export class BezierMouseEngine extends EventEmitter {
  ...
  private noisePhase: number = Math.random() * 1000;
  private noiseStep: number = 0.02 + Math.random() * 0.03;

  constructor(config?: Partial<BezierConfig>) {
    super();
    this.config = {
   ...
   */
  public generatePath(from: Point, to: Point): MovementStep[] {
    this.totalMoves++;
   ...
   */
  public async moveAndClick(page: any, target: Point, options?: {
    clickDelay?: number;
    doubleClick?: boolean;
    ...
    if (options?.doubleClick) {
      ...
      await page.mouse.move(
        target.x + this.jitter(this.getFatigueFactor()) * 0.2,
        target.y + this.jitter(this.getFatigueFactor()) * 0.2
      );
   ...
   */
  public async humanScroll(page: any, deltaY: number, options?: {
    steps?: number;
    smooth?: boolean;
  }): Promise<void> {
   ...
   */
  public async idleDrift(page: any, durationMs: number, center: Point): Promise<void> {
   ...
   */
  public setNervousness(value: number): void {
   ...
   */
  private cubicBezierPath(from: Point, to: Point, distance: number, fatigue: number): MovementStep[] {
    ...
    for (let i = 1; i <= numSteps; i++) {
      ...
      // Determine phase
      let phase: MovementStep['phase'];
      if (t < 0.2) phase = 'accelerate';
      else if (t > 0.85) phase = 'decelerate';
      else phase = 'cruise';
   ...
   */
  private correctionPath(from: Point, to: Point, fatigue: number): MovementStep[] {
    const distance = this.distance(from, to);
   ...
   */
  private randomControlPoint(from: Point, to: Point, position: number): Point {
   ...
   */
  private computeOvershootPoint(from: Point, to: Point): Point {
    const dx = to.x - from.x;
    const dy = to.y - from.y;
    const dist = Math.sqrt(dx * dx + dy * dy);
   ...
   */
  private jitter(fatigue: number): number {
  ...
  private easeInOutSpeed(t: number): number {
    // Bell curve: slow → fast → slow
    if (t < 0.2) return 0.3 + t * 3.5; // Accelerate
    if (t > 0.8) return 0.3 + (1 - t) * 3.5; // Decelerate
...
let mouseEngine: BezierMouseEngine | null = null;

export function getBezierMouseEngine(config?: Partial<BezierConfig>): BezierMouseEngine {
  if (!mouseEngine) {
...
export default BezierMouseEngine;
