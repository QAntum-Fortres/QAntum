# 🧠 @aeterna/pinecone-bridge

> **"52,573+ Vectors. Eternal Context. Zero Memory Loss."**
> 
> The cognitive core of the Aeterna Empire - providing eternal memory and autonomous intelligence.

## 📦 Package Overview

This package provides the **Pinecone Context Bridge** - a sophisticated system that connects the Aeterna Empire to Pinecone's vector database, enabling:
...
│       └── AeternaOrchestrator.ts    # Unified entry (~600 lines)
├── package.json
├── tsconfig.json
└── README.md
```
...
```bash
npm install @aeterna/pinecone-bridge
```

### Basic Usage

```typescript
import { BridgeSystem, createAeternaOrchestrator } from '@aeterna/pinecone-bridge';

// Create and initialize
const bridge = new BridgeSystem();
const aeterna = createAeternaOrchestrator(bridge, { autoStart: true });

// Query eternal context
const context = await aeterna.queryContext("authentication flow");
console.log(`Found ${context.totalRelevant} relevant vectors`);

// Make AI decision with historical context
const decision = await aeterna.makeDecision(
...
const analysis = await aeterna.meditate("system performance");
...
### 2. AeternaOrchestrator

Unified entry point for all daemon capabilities:

```typescript
const orchestrator = createAeternaOrchestrator(bridge, {
  autoStart: true,
  debug: true
});
...
PINECONE_INDEX=aeterna-empire
AETERNA_SESSION_ID=optional-session-id
```
