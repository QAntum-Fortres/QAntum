...
import { WorkerTask, WorkerInfo, WorkerPoolConfig, WorkerPoolStats } from '../types';
...
export class WorkerPoolManager extends EventEmitter {
  ...
  private spawnWorker(): Worker {
    ...
    const isFilePath =
      this.config.workerScript.endsWith('.js') ||
  ...
  private handleWorkerMessage(
    workerId: number,
    message: { type: string; taskId?: string; result?: unknown; duration?: number; error?: string }
  ): void {
    ...
    return new Promise<R>((resolve, reject) => {
      ...
      const timeoutHandle = setTimeout(() => {
        if (
          ...
          this.taskQueue.toArray().some((t) => t.id === task.id)
        ) {
  ...
  getStats(): WorkerPoolStats {
    ...
    return {
      ...
      avgTaskTime: this.tasksCompleted > 0 ? this.totalExecutionTime / this.tasksCompleted : 0,
...
export function workerMain(): void {
  ...
  parentPort!.on(
    'message',
    async (message: { type: string; taskId?: string; taskType?: string; payload?: unknown }) => {
      if (message.type === 'executeTask') {
