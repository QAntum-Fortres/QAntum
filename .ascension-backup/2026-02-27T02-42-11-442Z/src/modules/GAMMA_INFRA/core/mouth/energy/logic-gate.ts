...
const DANGEROUS_PATTERNS = [
  ...
  // Memory issues
  {
    pattern: /new Array\s*\(\s*\d{7,}\s*\)/,
    name: 'huge array allocation',
    severity: 'high' as const,
  },
  {
    pattern: /\.push\s*\([^)]*\)\s*;?\s*\}/,
    name: 'unbounded push in loop',
...
export class AILogicGate extends EventEmitter {
   ...
   */
  async validate(code: string, context: Record<string, unknown> = {}): Promise<ValidationReport> {
  ...
  private calculateMetrics(code: string): Omit<CodeMetrics, 'dangerousPatterns'> {
    ...
    const branchKeywords = code.match(
      /\bif\b|\belse\b|\bfor\b|\bwhile\b|\bcase\b|\bcatch\b|\b\?\b|\b&&\b|\b\|\|\b/g
    );
