...
import { SwarmDataInjector } from '../../../../../synthesis/swarm-data-injection.ts';
...
`);
    ...
    private async runPhase(phase: string): Promise<PhaseResult> {
        ...
        try {
            ...
            // 🧬 LOAD INJECTION: Generate realistic data for data/api phases
            if (phase.includes('data') || phase.includes('api') || phase.includes('swarm') || phase.includes('ghost')) {
                const injector = SwarmDataInjector.getInstance();
                // Generate varying amount of complex data based on loadFactor
                const injectionCount = Math.floor(10 * loadFactor) + 1;
                for (let i = 0; i < injectionCount; i++) {
                    injector.generateWorkerData(`stress_load_${phase}_${startTime}_${i}`);
                }
            }
