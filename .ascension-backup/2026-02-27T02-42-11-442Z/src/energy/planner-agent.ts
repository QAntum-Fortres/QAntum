...
export class PlannerAgent extends BaseAgent {
  ...
  async revisePlan(
    ...
    // Create revised plan
    const revisedTasks = this.reviseTasks(
      originalPlan.tasks,
      feedback,
    ...
    if (goalLower.includes('login') || goalLower.includes('authenticate')) {
      tasks.push(
        ...
        this.createTask(
          'fill',
          'username field',
          { field: 'username', value: context?.username },
          tid
        ),
        this.createTask(
          'fill',
          'password field',
          { field: 'password', value: context?.password },
          tid
        ),
      ...
      tasks.push(
        ...
        this.createTask(
          'extract',
          context?.selector || 'data elements',
   ...
   */
  private reviseTasks(originalTasks: SwarmTask[], feedback: string, traceId: string): SwarmTask[] {
