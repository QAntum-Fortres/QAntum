...
export class TLSPhantomStrategy implements IStealthStrategy {
  ...
  private fingerprints = ['Chrome/120.0.0.0', 'Chrome/119.0.0.0', 'Firefox/121.0', 'Safari/17.2'];
  ...
  async apply(page: Page): Promise<void> {
    ...
    await page.addInitScript((fingerprint) => {
      ...
      Object.defineProperty(navigator, 'userAgent', {
        get: () =>
          `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) ${fingerprint}`,
      });
    }, fp);
  }
...
export class GhostOrchestrator extends EventEmitter {
  ...
  async applyAll(page: Page): Promise<ApplyResult> {
    ...
    const totalRisk =
      results.reduce((sum, r) => sum + (r.success ? r.riskScore : 100), 0) / results.length;
  ...
  getStealthScore(): number {
    ...
    const avgRisk =
      enabledStrategies.reduce((sum, s) => sum + s.getRiskScore(), 0) / enabledStrategies.length;
