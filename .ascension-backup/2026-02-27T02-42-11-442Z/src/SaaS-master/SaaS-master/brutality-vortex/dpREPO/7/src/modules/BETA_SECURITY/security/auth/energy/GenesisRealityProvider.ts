...
// ═══════════════════════════════════════════════════════════════════════════════
// TYPES - GENESIS AXIOM MAPPINGS
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Genesis Axiom Types that affect environment configuration
 */
export type GenesisAxiomType =
  | 'IDENTITY'           // Unique container isolation
  | 'CAUSALITY'          // Service dependencies
  | 'TEMPORAL'           // Time-related behaviors
  | 'CONSERVATION'       // Resource limits
  | 'SYMMETRY'           // Load balancing
  | 'COMPLEMENTARITY'    // A/B testing scenarios
  | 'UNCERTAINTY'        // Chaos engineering
  | 'EMERGENCE'          // Auto-scaling rules
  | 'HOLOGRAPHIC';       // Distributed state sync

/**
 * Causality Types that define service relationships
 */
export type GenesisCausalityType =
  | 'DETERMINISTIC'      // Strict dependency order
  | 'PROBABILISTIC'      // Random failures
  | 'RETROCAUSAL'        // State reversal capability
  | 'ACAUSAL'            // Independent services
  | 'SUPERDETERMINISTIC' // Fully controlled
  | 'BIDIRECTIONAL'      // Two-way sync
  | 'EMERGENT';          // Self-organizing

/**
 * Reality specification - what the Genesis engine produces
 */
export interface GenesisRealitySpec {
  id: string;
  name: string;
  dimensions: number;
  axioms: GenesisAxiom[];
  causality: GenesisCausalityType;
  coherence: number;        // 0-1, how stable the reality is
  entropy: number;          // 0-1, chaos level
  temporalFlow: 'FORWARD' | 'BIDIRECTIONAL' | 'STATIC';
  createdAt: Date;
  tenantId: string;
}

export interface GenesisAxiom {
  id: string;
  type: GenesisAxiomType;
  statement: string;
  strength: number;         // 0-1
  dependencies: string[];   // Other axiom IDs
  metadata?: Record<string, any>;
}

/**
 * Docker Compose service definition generated from axioms
 */
...
export interface DockerComposeConfig {
  version: string;
  services: Record<string, DockerServiceSpec>;
  networks?: Record<string, { driver?: string; external?: boolean }>;
  volumes?: Record<string, { driver?: string }>;
}

/**
 * Running reality instance
 */
export interface ManifestatedReality {
  id: string;
  specId: string;
  tenantId: string;
  status: 'MANIFESTING' | 'STABLE' | 'UNSTABLE' | 'COLLAPSING' | 'COLLAPSED';
...
/**
 * Observation request - test execution within reality
 */
...
export class GenesisRealityProvider extends EventEmitter {
  private prisma: PrismaClient;
  private realitiesDir: string;
  private activeRealities: Map<string, ManifestatedReality> = new Map();
  private realityProcesses: Map<string, ChildProcess> = new Map();

  // Default images for different service types
  private readonly serviceImages: Record<string, string> = {
    web: 'nginx:alpine',
    api: 'node:20-alpine',
  ...
  constructor(options: { realitiesDir?: string } = {}) {
    super();
    this.prisma = new PrismaClient();
    this.realitiesDir = options.realitiesDir || join(process.cwd(), '.genesis-realities');
    this.ensureRealitiesDir();
    this.setupShutdownHandlers();
  }

  private async ensureRealitiesDir(): Promise<void> {
    if (!existsSync(this.realitiesDir)) {
      await mkdir(this.realitiesDir, { recursive: true });
    }
  }
   ...
   */
  translateAxiomsToDocker(spec: GenesisRealitySpec): DockerComposeConfig {
    const services: Record<string, DockerServiceSpec> = {};
   ...
   */
  private axiomToService(
    axiom: GenesisAxiom,
    spec: GenesisRealitySpec,
    network: string
  ): DockerServiceSpec | null {
      ...
      case 'CAUSALITY':
        ...
        return {
          ...
          depends_on: axiom.dependencies.length > 0
            ? Object.fromEntries(
                axiom.dependencies.map(dep => [dep, { condition: 'service_healthy' }])
              )
            : undefined,
          labels: {
            'genesis.axiom.type': axiom.type,
   ...
   */
  private applyCausalityToServices(
    services: Record<string, DockerServiceSpec>,
    causality: GenesisCausalityType
  ): void {
   ...
   */
  async manifestReality(spec: GenesisRealitySpec): Promise<ManifestatedReality> {
    const realityDir = join(this.realitiesDir, spec.id);
    ...
    const composeFile = join(realityDir, 'docker-compose.yml');

    // Write compose file
    await writeFile(composeFile, this.toYaml(composeConfig));

    // Create manifestation record
    const manifestation: ManifestatedReality = {
      id: randomUUID(),
      specId: spec.id,
   ...
   */
  async collapseReality(realityId: string): Promise<void> {
    ...
    const realityDir = join(this.realitiesDir, manifestation.specId);
    ...
    try {
      ...
      // Cleanup directory
      await rm(realityDir, { recursive: true, force: true });
   ...
   */
  async observeReality(request: ObservationRequest): Promise<ObservationResult> {
    ...
    try {
      ...
      const targetContainer = manifestation.containers.find(
        c => c.name.includes(request.targetService)
      );
      if (!targetContainer) {
  ...
  private async waitForStability(realityId: string, realityDir: string): Promise<void> {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      try {
        ...
        const allHealthy = containers.every(
          (c: any) => c.State === 'running' && (!c.Health || c.Health === 'healthy')
        );
  ...
  private async getContainerInfo(realityDir: string): Promise<ContainerInfo[]> {
    try {
      const { stdout } = await this.runDockerCompose(realityDir, 'ps', ['--format', 'json']);
      ...
      return stdout.trim().split('\n')
        ...
        .map(line => {
          ...
          return {
            ...
            image: c.Image,
            status: c.State,
            ports: c.Ports?.split(',').filter((p: string) => p) || [],
            health: c.Health as any,
          };
        });
    } catch {
      return [];
  ...
  private async markRealityCollapsed(realityId: string): Promise<void> {
    await this.prisma.$executeRaw`
   ...
   */
  getActiveRealities(tenantId?: string): ManifestatedReality[] {
   ...
   * Get reality by ID
   */
  getReality(realityId: string): ManifestatedReality | undefined {
...
export const genesisRealityProvider = new GenesisRealityProvider();
