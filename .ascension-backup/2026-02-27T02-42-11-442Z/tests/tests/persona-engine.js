...
exports.personaEngine = exports.PersonaEngine = exports.PERSONA_TEMPLATES = void 0;
const events_1 = require("events");
/**
 * Pre-defined persona templates
 */
exports.PERSONA_TEMPLATES = {
...
class PersonaEngine extends events_1.EventEmitter {
    ...
    loadPersona(personaOrTemplate) {
        if (typeof personaOrTemplate === 'string') {
            const template = exports.PERSONA_TEMPLATES[personaOrTemplate];
            if (!template) {
     ...
     */
    calculateBehaviorProfile(persona) {
        const { techSavviness, patienceLevel, visualImpairment, interactionSpeed } = persona;
        // 🔥 Rage click threshold: Low patience = faster rage
        // Base: 5000ms, Min: 500ms for extremely impatient users
        const rageClickThreshold = Math.max(500, 5000 - (100 - patienceLevel) * 45);
        ...
        const interactionDelay = Math.floor(baseDelay + (speedFactor * 800) + (techFactor * 600));
        // ⌨️ Typing speed: 20-120 CPM based on speed and tech
        const typingSpeed = Math.floor(20 + (interactionSpeed * 0.5) + (techSavviness * 0.5));
        // 🎯 Mouse precision: Lower = more variance
        // Base: 2px, Max: 20px for low precision
        const mousePrecision = Math.floor(2 + ((100 - techSavviness) * 0.1) + (visualImpairment * 0.08));
        // 📖 Reading speed: 100-400 WPM
        const readingSpeed = Math.floor(100 + (techSavviness * 2) + (interactionSpeed * 1));
        // ❌ Miss-click probability: Higher for impaired/low-tech
        const missClickProbability = Math.min(0.3, (visualImpairment / 500) + ((100 - techSavviness) / 500));
        ...
        const doubleClickProbability = Math.min(0.15, ((100 - techSavviness) / 1000) +
     ...
     */
    shouldRageClick(waitTimeMs) {
        if (!this.behaviorProfile || !this.currentPersona)
            return false;
     ...
     */
    generateRageClicks(targetX, targetY) {
     ...
     */
    shouldMissClick(targetWidth, targetHeight) {
        if (!this.behaviorProfile)
            return false;
    ...
    generateUXRecommendations() {
        ...
        if (metrics.rageClicks > 0) {
            recommendations.push(`⚠️ Detected ${metrics.rageClicks} rage click events. ` +
        ...
        if (metrics.missClicks > 0) {
            recommendations.push(`🎯 Detected ${metrics.missClicks} miss-click events. ` +
        ...
        if (this.currentPersona.ageGroup === 'senior') {
            recommendations.push('👴 Consider larger font sizes (16px+) and simpler navigation for senior users.');
        }
     ...
     */
    static getTemplates() {
        return Object.keys(exports.PERSONA_TEMPLATES);
    }
    /**
     * Create custom persona from partial configuration
     */
    static createCustomPersona(name, config) {
        return {
            id: `custom-${Date.now()}`,
            name,
