# QAntum Knox Validator & Vault Signer
...
```mermaid
graph TD
    Node["QAntum Engine (Node.js)"] --> Bridge["Termux Bridge (Port 3002)"]
...
## 🛠️ Implementation Details (`qantum/KnoxVaultSigner.ts`)
...
```typescript
import { KnoxVaultSigner } from './qantum/KnoxVaultSigner';
...
All signatures are tracked in `SigningResult.auditHash`, creating a cryptographically linked history of every decision made by the QAntum Engine.
