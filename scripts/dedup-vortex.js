...
function getSourcePriority(filename) {
    ...
    if (filename.includes('AETERNA-WEB-CORE')) return 4;
    if (filename.includes('AETERNA-NEW')) return 4;
...
function extractCanonicalName(filename) {
    ...
    const noisePatterns = /^(C|Users|papic|Downloads|Desktop|MAGICSTICK|Mind-Engine-Core|QAntum.*|MisteMind.*|MrMind.*|ManualQATester|MisterMind.*|AETERNA.*|jules.*|TheArchitect|ALLIN.*|tools|scripts|src|tests|bug-reports|docs|TRAINING|PRIVATE|PROJECT|dpengeneering|cybercody_.*|SCRIPTS|purge-backup|ascension-backup|ARCHIVE|security_core|modules|apps|packages|api|worker)$/i;
