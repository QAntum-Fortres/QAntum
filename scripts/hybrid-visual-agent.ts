 ...
 *   4. Email build  → screenshots embedded + video link + aeterna.website offer
...
// Aeterna offer details
const AETERNA = {
    site: 'https://aeterna.website',
    portal: 'https://aeterna.website/portal.html',
    demo: 'https://aeterna.website/demo.mp4',
    calendar: 'https://calendly.com/dimitar-prodromov/30min',
    screenshot: 'https://aeterna.website/screenshots/dashboard.png',
    logo: 'https://aeterna.website/logo.png',
...
const ALL_TARGETS: Target[] = [
    { domain: 'ozone.bg', email: 'privacy@ozone.bg', company: 'Ozone', industry: 'E-commerce' },
    ...
    try {
      ...
      `;
        ...
        <div style="font-size:11px;color:#6060a0;border-top:1px solid #2a2a50;padding-top:8px">
          Scanned by aeterna.website · ${new Date().toLocaleTimeString()}
        </div>
      `;
      ...
      `;
        ...
        <div style="margin-top:12px;font-size:11px;color:#5050a0">
          Fix available at aeterna.website · All data from public HTTP headers — same as SecurityHeaders.com
        </div>
      `;
...
<div style="max-width:700px;margin:0 auto;padding:32px 16px">
  ...
  <div style="background:#0c180c;border:1px solid #1a5a1a;border-radius:12px;padding:20px;margin-bottom:24px">
    ...
    <a href="${AETERNA.demo}" style="display:inline-block;background:linear-gradient(135deg,#1a4a1a,#0a2a0a);border:1px solid #2a7a2a;color:#80d080;text-decoration:none;padding:12px 24px;border-radius:8px;font-size:14px;font-weight:600">
  ...
  <div style="background:linear-gradient(135deg,#160840,#0c1640);border:2px solid #6d3af0;border-radius:16px;padding:36px;text-align:center;margin-bottom:28px">
    ...
    <p style="font-size:14px;color:#9090c0;margin-bottom:6px;line-height:1.7">
      <strong style="color:#a78bfa">Aeterna</strong> by QAntum monitors your security headers 24/7,
    ...
    <div style="display:flex;justify-content:center;gap:12px;flex-wrap:wrap">
      <a href="${AETERNA.site}" style="display:inline-block;background:linear-gradient(135deg,#6d3af0,#3d1aa0);color:#fff;text-decoration:none;padding:16px 36px;border-radius:10px;font-weight:800;font-size:15px">
        See aeterna.website →
      </a>
      <a href="${AETERNA.calendar}" style="display:inline-block;border:1px solid #5050a0;color:#a0a0d0;text-decoration:none;padding:16px 36px;border-radius:10px;font-weight:600;font-size:15px">
        Book 30-min call
      </a>
    </div>
    <div style="margin-top:16px">
      <a href="${AETERNA.portal}" style="color:#5050a0;font-size:13px;text-decoration:none">or start free self-service audit →</a>
    </div>
  </div>
  ...
  <div style="text-align:center;font-size:12px;color:#303050;border-top:1px solid #111128;padding-top:20px">
    ...
    <a href="mailto:${AETERNA.email}" style="color:#404080">${AETERNA.email}</a>
    · <a href="${AETERNA.site}" style="color:#404080">aeterna.website</a><br><br>
...
function getTransport(): nodemailer.Transporter {
    if (!_transport) {
        _transport = nodemailer.createTransport({
            service: 'gmail',
            auth: { user: AETERNA.authUser, pass: process.env.GMAIL_APP_PASSWORD },
        });
    }
    return _transport;
}
...
async function main(): Promise<void> {
    ...
    // Select targets
    const targets = TARGET_ARG
        ? ALL_TARGETS.filter(t => t.domain === TARGET_ARG)
        : ALL_TARGETS;

    if (targets.length === 0) {
        console.error(`❌ Target "${TARGET_ARG}" not found. Available: ${ALL_TARGETS.map(t => t.domain).join(', ')}`);
        process.exit(1);
    }
