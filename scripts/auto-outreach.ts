...
const ASSETS = {
  landingPage: 'https://aeterna.website',
  demoVideoUrl: 'https://aeterna.website/demo.mp4',
  dashboardScreenshot: 'https://aeterna.website/screenshots/dashboard.png',
  reportGif: 'https://aeterna.website/screenshots/report-preview.gif',
  portalPage: 'https://aeterna.website/portal.html',
...
const TARGETS: Target[] = [
  { domain: 'ozone.bg', contactEmail: 'privacy@ozone.bg', companyName: 'Ozone', industry: 'ecommerce' },
  { domain: 'chaos.com', contactEmail: 'security@chaos.com', companyName: 'Chaos', industry: 'saas' },
  { domain: 'telerik.com', contactEmail: 'info@telerik.com', companyName: 'Telerik', industry: 'saas' },
  { domain: 'superhosting.bg', contactEmail: 'support@superhosting.bg', companyName: 'Superhosting', industry: 'hosting' },
  { domain: 'kanbanize.com', contactEmail: 'hello@kanbanize.com', companyName: 'Kanbanize', industry: 'saas' },
  { domain: 'payhawk.com', contactEmail: 'hello@payhawk.com', companyName: 'Payhawk', industry: 'fintech' },
  { domain: 'jobs.bg', contactEmail: 'office@jobs.bg', companyName: 'Jobs.bg', industry: 'recruitment' },
  { domain: 'gtmhub.com', contactEmail: 'hello@gtmhub.com', companyName: 'Gtmhub', industry: 'saas' },
  { domain: 'officerentinfo.bg', contactEmail: 'office@officerentinfo.bg', companyName: 'OfficeRentInfo', industry: 'other' },
  { domain: 'siteground.com', contactEmail: 'security@siteground.com', companyName: 'SiteGround', industry: 'hosting' },
];
...
<div style="max-width:680px;margin:0 auto;padding:32px 20px">
  ...
  <div style="background:linear-gradient(135deg,#18083c,#0c1640);border:1px solid #5d3dbf;border-radius:14px;padding:32px;text-align:center;margin-bottom:28px">
    ...
    <div style="margin-top:14px">
      <a href="${ASSETS.portalPage}" style="color:#6060a0;font-size:13px;text-decoration:none">or start self-service audit →</a>
    </div>
  </div>
  ...
  <div style="text-align:center;font-size:12px;color:#404060;border-top:1px solid #151530;padding-top:20px">
    ...
    · <a href="${ASSETS.landingPage}" style="color:#4040a0">aeterna.website</a><br>
    <br>
...
function getTransporter(): nodemailer.Transporter {
  if (!transporter) {
    transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.GMAIL_FROM_EMAIL || process.env.GMAIL_EMAIL,
...
async function sendEmail(scan: ScanResult): Promise<{ ok: boolean; id?: string; err?: string }> {
  const subject = `Quick security win for ${scan.domain} — ${scan.missingHeaders.length} issues found (Grade ${scan.grade})`;
  const html = buildHtmlEmail(scan);

  try {
    const info = await getTransporter().sendMail({
      from: `"Dimitar @ QAntum" <${process.env.GMAIL_FROM_EMAIL || process.env.GMAIL_EMAIL}>`,
      to: scan.contactEmail,
      subject,
      html,
      replyTo: process.env.GMAIL_FROM_EMAIL || process.env.GMAIL_EMAIL,
      text: `Hello ${scan.companyName},\n\nWe analyzed ${scan.domain}: ${scan.missingHeaders.length} security gaps, Grade ${scan.grade}.\n\nFree fix + full report: ${ASSETS.landingPage}\nDemo video: ${ASSETS.demoVideoUrl}\n\nDimitar Prodromov · dp@qantum.site`,
    });
  ...
  for (let i = 0; i < prioritized.length; i++) {
    ...
    if (!DRY_RUN) {
      ...
      if (result.ok) {
        ...
        notifyPhone(`✅ Email sent to ${scan.companyName} (${scan.domain}) Grade ${scan.grade}`, 'INFO');
        appendLog({ domain: scan.domain, to: scan.contactEmail, score: scan.score, grade: scan.grade, sent: true, id: result.id });
      } else {
        failed++;
        console.log(`    ❌ Failed: ${result.err}`);
        notifyPhone(`❌ Send failed: ${scan.domain} — ${result.err}`, 'WARNING');
        appendLog({ domain: scan.domain, to: scan.contactEmail, score: scan.score, grade: scan.grade, sent: false, err: result.err });
      }
